...
def test_format_action_omits_none_ids() -> None:
    line = format_action("remove", dry_run=True, tmdb=5, tvdb=None, title="X")
...
def test_log_action_emits_info(capsys) -> None:
    ...
    configure_logging("INFO")
    log_action(get_logger("test"), "requested", dry_run=False, tmdb=10)
    captured = capsys.readouterr().err
    assert "action=requested" in captured
    assert "dry_run=false" in captured

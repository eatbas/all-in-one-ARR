...
def seed(db) -> None:
    db.upsert_item(
        **{k: _MOVIE[k] for k in _MOVIE if k != "list_id"}, list_id="watchlist"
    )
    db.upsert_item(
        **{k: _SHOW[k] for k in _SHOW if k != "list_id"}, list_id="watchlist"
    )
...
async def test_removes_movie_by_tmdb(db) -> None:
    ...
    assert any(
        'Removed "Dune" from the Trakt list.' in a["detail"]
...
async def test_removes_known_seer_request_without_touching_arr_media(db) -> None:
    ...
    assert any(
        'Removed the Seer request for "Dune".' in a["detail"]
        for a in db.recent_activity()
    )
...
async def test_request_delete_failure_leaves_item_active(db) -> None:
    ...
    assert any(
        a["action"] == "Removal failed"
        and "Could not remove the Seer request" in a["detail"]
        for a in db.recent_activity()
    )

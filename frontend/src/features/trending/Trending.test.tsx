...
import { TRENDING_TAB_STORAGE_KEY } from "@/features/trending/trending-tab"
...
describe("Trending", () => {
  ...
  it("hides items already in the library or available in Seer when toggled", async () => {
    vi.mocked(useTrending).mockReturnValue(
      queryResult([
        ITEM,
        { ...ITEM, tmdb: 2, title: "Owned", in_library: true },
        { ...ITEM, tmdb: 3, title: "Avail", seer_status: 5 },
      ]),
    )
    const user = userEvent.setup()
    render(<Trending />)
  ...
  it("shows an all-available message when the filter hides every result", async () => {
    vi.mocked(useTrending).mockReturnValue(
      queryResult([{ ...ITEM, in_library: true }]),
    )
    const user = userEvent.setup()
    render(<Trending />)
    await user.click(screen.getByRole("switch", { name: "Hide available items" }))
